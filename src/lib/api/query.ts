const normalizeSort = (order: any): any => {
	...
	const field = Object.getOwnPropertyNames(order)[0];
	const parsed = field.split('.');

	if (parsed.length < 1) return order;
...
const normalizeFilterStr = (field: string, value: any): any => {
	...
	if (parsed.length <= 1) return { [field]: value };
...
const normalizeFilter = (filter: any) => {
	...
	if (Array.isArray(filter)) {
		...
		for (let i = 0; i < props.length; i++) {
			const propName: string = props[i].toString();
			const value: any = filter[propName];

			if (propName.includes('.')) f = { ...f, ...normalizeFilterStr(propName, value) };
			else f[propName] = normalizeFilter(value);
		}
	}
